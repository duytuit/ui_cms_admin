  ...
  useEffect(() => {
    ...
    let baocaoloinhuan = (data[0]?.data?.debit_theoxe || []).map((row: any) => {
      ...
      return {
        ...row,
        totalCost,
        phiDau :listPhi?.reduce((sum: number, p: any) => p.income_expense_category_id === 17 ? sum + (p.amount ? parseFloat(p.amount.toString()) : 0) : sum, 0) || 0,
        cuocDuongBo : listPhi?.reduce((sum: number, p: any) => p.income_expense_category_id === 18 ? sum + (p.amount ? parseFloat(p.amount.toString()) : 0) : sum, 0) || 0,
        phiSuaChua : listPhi?.reduce((sum: number, p: any) => p.income_expense_category_id === 16 ? sum + (p.amount ? parseFloat(p.amount.toString()) : 0) : sum, 0) || 0,
        phiKhac : listPhi?.reduce((sum: number, p: any) => p.income_expense_category_id === 19 ? sum + (p.amount ? parseFloat(p.amount.toString()) : 0) : sum, 0) || 0,
        laiVay : listPhi?.reduce((sum: number, p: any) => p.income_expense_category_id === 20 ? sum + (p.amount ? parseFloat(p.amount.toString()) : 0) : sum, 0) || 0,
        trichLuongLaiXe : listPhi?.reduce((sum: number, p: any) => p.income_expense_category_id === 22 ? sum + (p.amount ? parseFloat(p.amount.toString()) : 0) : sum, 0) || 0,
        trichBHXH : listPhi?.reduce((sum: number, p: any) => p.income_expense_category_id === 34 ? sum + (p.amount ? parseFloat(p.amount.toString()) : 0) : sum, 0) || 0,
    ...
    const loinhuanhaiquan = [
      {
        ...
        loiNhuan: (data[2]?.data.reduce((acc: number, curr: any) => acc + curr.total_price, 0)) - 
        (data[2]?.data.reduce((acc: number, curr: any) => acc + curr.total_purchase_price, 0) || 0),
      }
    ]
