...
export default function UpdateFileGia({ id, onClose }: { id: any; onClose: () => void }) {
  ...
  const [newDebit, setNewDebit] = useState<any>({ name: "", purchasePrice: 0, price: 0,vat:0, note: "", bill: "" ,SupplierDetailId:"",SupplierName:""});
  ...
  async function fetchDebitDetail(id: number, partnerOptions: any[], setInfos: any, setDebitDetail: any, setLoading: any) {
    ...
    try {
      ...
      let _list_giaonhan = '';
      if (detail.fileInfoDetails) {
          detail.fileInfoDetails.forEach((element: any, index: number) => {
              const _employee = listEmployee.find((x: any) => x.id == element.employeeId);
              const name = `${_employee?.last_name ?? ""} ${_employee?.first_name ?? ""}`.trim();

              if (index > 0) _list_giaonhan += '; ';
              _list_giaonhan += name;
          });
      }
            ...
            <table className="w-full">
              <tbody>
                ...
                <tr>
                  ...
                  <td className="pr-4 align-top">
                     <div className="mb-2">
                      <label className="font-medium mr-2">Giao nhận:</label>
                      <span>{infos.list_giaonhan}</span>
                    </div>
                  </td>
                </tr>
                {/* --- HÀNG 3 --- */}
