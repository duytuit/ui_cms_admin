...
import { useListDebit } from "../service";
...
export default function ListDebit() {
    ...
    const { data, loading, error, refresh } = useListDebit({
    ...
    useEffect(() => {
        ...
        handleParamUrl(paramsPaginator);
         const mapped = (data?.data || []).map((row: any) => {
                    const type = typeDebit.find((x: any) => x.isExternalDriver === row.is_external_driver);
                    return {
                        ...row,
                        type:type?.name || "",
                    };
