...
const UpdateCampaign = () => {
    ...
    return (
        <AddForm className="w-8" style={{ margin: '0 auto' }} checkId={infos.id} title='chiến dịch'
            loading={loading} onSubmit={handleSubmit} route={Number(id) ? '/campaign/update' : '/campaign/add'} >
            <div style={{ backgroundColor: '#f8f9fa' }} className="card">
                <div className="grid formgrid">
                    <div className="col-12 lg:col-12">
                        <InputForm id='name' value={infos.name} onChange={(e:any) => setInfos({ ...infos, name: e.target.value })} label='Tên chiến dịch' required />
                        <DropdownForm label='Công ty' filter optionValue="id" optionLabel="name" value={infos.company_id}
                            onChange={(e:any) => { setInfos({ ...infos, company_id: e.target.value }); setError1(false) }}
                            options={[]} className={classNames({ 'p-invalid': error1 })} disabled={infos.round} />
                        <DropdownForm label='Dự án' filter optionValue="id" optionLabel="cb_title" disabled={infos.round} value={infos.category_id}
                            onChange={(e:any) => { setInfos({ ...infos, category_id: e.target.value }); setError1(false) }}
                            options={infos.company_id ? [] : []} className={classNames({ 'p-invalid': error1 })} />
                        <DropdownForm label='Nguồn' filter optionValue="id" value={infos.source_id} disabled={infos.round}
                            onChange={(e:any) => { setInfos({ ...infos, source_id: e.target.value }); setError2(false) }}
                            options={infos.company_id ? [] : []} className={classNames({ 'p-invalid': error2 })} />
                        ...
                        <div className="flex align-items-center mb-3">
                            ...
                            <MultiSelect value={infos.user_id_manager} filter options={[]}
                                onChange={(e) => (setInfos({ ...infos, user_id_manager: e.target.value }), setError3(false))}
                                optionLabel="full_name" optionValue="user_id"
                                placeholder="Chọn người quản lý chiến dịch" display="chip" className={classNames('w-9', { 'p-invalid': error3 })} />
                        </div>
                        <InputForm id='rule_time' value={infos.rule_time} type='number' label='Thời gian chăm sóc quy định (phút)'
                            onChange={(e:any) => setInfos({ ...infos, rule_time: e.target.value })} required />
                        <InputForm id='penalty' value={infos.penalty} type='number' label='Quy định phạt (tour)'
                            onChange={(e:any) => setInfos({ ...infos, penalty: e.target.value })} required />
                    </div>
                </div>
            </div>
        </AddForm>
